...
function countUnseenTasksForTabs(tasks: any[], userId: string) {
  return tasks.reduce(
    (count, task) => (task.seenBy.includes(userId) ? count : count + 1),
    0
  );
}
...
function countUnseenTasks(tasks: any[]) {
  ...
  return tasks.reduce(
    (count, task) => (!task.isSeenByMe ? count + 1 : count),
    0
  );
}
...
export const subtaskToIsTaskFromMe: Record<
  string,
  ...
  if (data) {
    return (
      ...
      data.map((item: any) => {
        ...
        return {
          label: `${label.toString()}  ${topic.toString()}`,
 ...
 * **/
export function addEventToTask(
  task: any,
  eventData: any,
  taskIndex: number
): void {
  if (taskIndex > -1) {
    ...
    const isUniqueEvent = task.events?.some(
      (event: any) => String(event._id) === String(eventData._id)
    );
    if (!isUniqueEvent) {
      task.events?.push(eventData);
      // sort task events by createdAt with latest on bottom
      const events = task.events?.sort(
        (a: any, b: any) =>
...
export function addUniqueEventToTask(task: any, forwardedTask: any): void {
  // console.log('existingEvents', task.events)
  const isUniqueEvent = task.events?.some(
    (event: any) => String(event._id) === String(forwardedTask._id)
  );
  if (!isUniqueEvent) {
    const updatedEvent = {
 ...
 * @param eventData
 * **/
export function updateTaskOnCancelEvent(
  task: any,
  taskIndex: any,
  eventData: any
) {
...
export const getTaskCardHeight = (task: ITask) => {
  try {
    const SINGLE_LINE_DESCRIPTION_CARD_HEIGHT = 103.5;
    const DOUBLE_LINE_DESCRIPTION_CARD_HEIGHT = 121;
    const taskHasFiles = task.files.length > 0;
    const taskHasDesc = task.description.length > 0;
    const TASK_WITH_FILES_AND_NO_DESC =
      taskHasFiles && task.description.length === 0;
    const TASK_WITH_FILES_AND_DESC = taskHasFiles && taskHasDesc;
    const NO_DESCRIPTION_CARD_HEIGHT = 85.5;
    const CHAR_COUNT_DESCRIPTION = 65;
    let height = 100;
    if (TASK_WITH_FILES_AND_NO_DESC || TASK_WITH_FILES_AND_DESC) {
      ...
      height =
        task.description.length > 0
          ? task.description.length > CHAR_COUNT_DESCRIPTION
            ? DOUBLE_LINE_DESCRIPTION_CARD_HEIGHT
            : SINGLE_LINE_DESCRIPTION_CARD_HEIGHT
          : NO_DESCRIPTION_CARD_HEIGHT;
    }
    return height;
  } catch (error) {
...
export const IS_IMAGE = (fileName: string) =>
  imageFileRegex.test(fileName.toLowerCase());
/**
 * @param fileType
 * @return return true or false
 * **/
 ...
 * **/
export const FILTER_DATA_BY_EXT = (
...
export const openFormWindow = (content: string) => {
  ...
  const formWindow: Window | null = window.open(
    undefined,
    "_blank",
    windowFeatures
  );
 ...
 * **/
export const combinedMemberArrayWithState = (
  membersArr: any[],
  state: any[]
) => {
  let combinedArray = membersArr.map((member: any) => {
    let tempState = "";
    state.every((user: any) => {
      if (String(member._id) === String(user.userId)) {
        tempState = user.userState;
        return false;
      }
      return true;
    });
