...
import { isTrue, momentdeDateFormat, momentTimeFormat } from 'components/Utills/Globals/Common';
...
export default function TaskDetail({ subtaskDetail, taskAdmin }: Props) {
    ...
    const [openRejectionDrawer, setOpenRejectionDrawer] = useState<boolean>(false)
    ...
    return (<>
            ...
            <CBox className={classes.wrapper}>
                ...
                <CustomStack justifyContent='flex-end' gap={2} >
                    {isTaskAdmin && <CBox display='flex' alignItems='center' mt={1} >
                        <CButton styles={{ fontSize: 14, textTransform: 'capitalize' }} onClick={handleRejectionDrawer} startIcon={<VisibilityOutlinedIcon />} label={'Rejections'} />
                    </CBox>}
                    <CBox display='flex' alignItems='center' justifyContent='flex-end' mt={1}>
                        <CButton styles={{ fontSize: 14, textTransform: 'capitalize' }} onClick={handleAttachmentDrawer} startIcon={<AttachmentIcon />} label={'Attachments'} />
                    </CBox>
                </CustomStack>
        ...
        <CDrawer
            ...
            children={
                <ViewRejectionComments subTaskHeading="Subtask Rejection" handleCloseCDrawer={handleCloseRejectionDrawer} cardData = {viewRejection} />
            } />
        <CDrawer
            showBoxShadow={true}
            hideBackDrop={true}
