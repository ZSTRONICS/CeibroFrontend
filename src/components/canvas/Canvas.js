...
class Canvas extends Component {
  ...
  componentDidMount() {
    const { id } = this.state;
    const { editable, canvasOption, workareaOption, guidelineOption } =
      this.props;
    ...
    const mergedWorkareaOption = Object.assign(
      ...
      workareaOption,
      { workareaWidth: canvasOption.width, workareaHeight: canvasOption.height }
    );
  ...
  handlers = {
    ...
    hideById: (id) => {
      ...
      if (findObject) {
        this.canvas.discardActiveObject();
        findObject.opacity = 0;
        findObject.selectable = false;
        this.canvas.requestRenderAll();
      }
    },
    copy: () => {
    ...
    /*
     ** convert and get the canvas as a image to be saved
     ** Terry Chan
     ** 17/04/2019
     */
    getCanvasBytes: () => {
      return new Promise((done) => {
        // fabric.js does not supports toBlob and converts to HTMLCanvasElement
  ...
  linkHandlers = {
    ...
    create: (link, init = false) => {
      ...
      const fromPort = fromNode.fromPort.filter(
        (port) => port.id === link.fromPort || !port.id
      )[0];
      const toNode = this.handlers.findById(link.toNode);
      const { toPort } = toNode;
      const createdObj = this.fabricObjects[link.type].create(
        fromNode,
        fromPort,
        toNode,
        toPort,
      ...
      if (target.tooltip && target.tooltip.enabled) {
        ...
        if (
          ...
          this.tooltipRef.style.left = `${
            left + offset.left - this.tooltipRef.offsetWidth
          }px`;
  ...
  eventHandlers = {
    ...
    keydown: (e) => {
      ...
      if ((e.keyCode === 46 && del) || e.keyCode === 8) {
  ...
  render() {
    ...
    return (
      ...
      >
        <canvas id={`canvas_${id}`} style={{ width: "100%", height: "100%" }} />
      </div>
    );
  }
}
