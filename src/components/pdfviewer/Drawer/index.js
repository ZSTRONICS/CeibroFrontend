...
class DrawerMenu extends Component {
  ...
  renderUploadErrorSnacker() {
    ...
    return (
      <Snackbar
        ...
        message={<span id="message-id">The chosen file is incorrect.</span>}
        action={[
          <IconButton
            key="close"
            aria-label="Close"
  ...
  renderFilePicker() {
    ...
    if (maxSize) {
      uploadCaption = `${uploadCaption} The file size of PDF should be less than equals to ${Math.floor(
        maxSize / 1000 / 1000
      )}MB.`;
    }
    return (
      ...
      >
        <DialogTitle id="alert-dialog-slide-title">{item.label}</DialogTitle>
        <DialogContent>
          <div className={this.props.classes.dialogContent}>
            <Typography color="inherit">{uploadCaption}</Typography>
          </div>
          <FilePicker
            options={options}
  ...
  renderDrawerIcon() {
    ...
    if (pdf) {
      containerStyles = [...containerStyles, classes.drawerIconEditing];
    }
    return (
  ...
  render() {
    return (
      <div>
        ...
        >
          <ListControl items={this.listItems} />
        </Drawer>
      </div>
    );
  }
